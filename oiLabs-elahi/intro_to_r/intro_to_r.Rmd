...
and RStudio is a convenient interface. Part of the beauty of RStudio is its seamless integration of document preparation using Rmarkdown, which will be covered at the very end of this document. 
...
The following code will create the same figure:
```{r, eval = FALSE}
with(arbuthnot, plot(girls ~ year))
```

What do you think the function `with` is doing?

And finally, we can use `~` syntax to recreate the same figure - read this as "the number of girls on the y-axis as a function of year on the x-axis":
```{r, eval = FALSE}
...
## Assignment 1 - due April 5, 5pm

### Your first assignment in R markdown 

We have been executing the above R code in an .Rmd document for practice. For a bit more detail, read the following websites:

https://rmarkdown.rstudio.com/articles_intro.html

https://r4ds.had.co.nz/r-markdown.html

For the assignment below, create a new .Rmd file entitled `birth_records_xxx.Rmd` where `xxx` is your last name - e.g., `birth_records_elahi.Rmd`. In this file, answer the questions about the present day birth records in the U.S. Include both text (the questions, and your answers), as well as the R code needed to produce the answers (as well as the answers). Knit the file to an html file (e.g., `birth_records_elahi.html`), and send it to Robin via email for your first lab assignment. 
...
## License
<div id="license">
This is a derivative product of OpenIntro, by Robin Elahi, that is released under a 
