...
For the exercises below, create a new .Rmd file entitled `brfss_xxx.Rmd` where `xxx` is your last name - e.g., `brfss_elahi.Rmd`. Include text (the questions, and your answers), as well as the R code needed to produce the answers. Knit the file to an html file (e.g., `brfss_elahi.html`), and send it to Robin via email. 
