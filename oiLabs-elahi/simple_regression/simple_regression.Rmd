...
We will include code that uses both `base R` as well as the `tidyverse` for comparison. 
...
load("oiLabs-elahi/simple_regression/more/mlb11.RData")
```
...
```{r, echo = FALSE}
mlb11 %>% ggplot(aes(at_bats, runs)) + geom_point()
```
...
The `base` function `abline` plots a line based on its slope and intercept. Here, we 
...
Using `ggplot2`: here we are literally adding a layer on top of our plot. `stat_smooth` creates the line by fitting a linear model. It can also show us the standard error `se`
...
Using `ggplot2`: after creating the scatterplot on the first layer (first two lines of code), we overlay a horizontal dashed line at $y = 0$ (to help us check whether residuals are distributed around 0), and we also adjust the axis labels to be more informative.
