...
For the exercises below, create a new .Rmd file entitled `kobe_xxx.Rmd` where `xxx` is your last name - e.g., `kobe_elahi.Rmd`. Include text (the questions, and your answers), as well as the R code needed to produce the answers. Knit the file to an html file (e.g., `kobe_elahi.html`), and send it to Robin via email. 
